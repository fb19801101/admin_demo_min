  ...
  <div class="layui-inline">
    ...
    <div class="layui-input-inline">
      <select name="conditionField" id="demoCondition" lay-verify="required" lay-search="">
        ...
        <option selected="true" value="dn_id">数量ID</option>
        <option value="dn_code">数量编码</option>
        ...
        <option value="dn_date">施工日期</option>
        <option value="dn_name">数量名称</option>
      </select>
    </div>
  </div>
  ...
  <form class="layui-form" id="formLayer" onsubmit="return false">
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="dnId" name="dnId" lay-verify="number" placeholder="可以为空，系统默认自增！" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">数量编码</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="dnCode" name="dnCode" lay-verify="required" autocomplete="off">
        </div>
      </div>
    </div>
        ...
        <div class="layui-input-inline">
          <select id="piId" name="piId" lay-verify="required" lay-search="" lay-filter="selectPartItem">
            ...
            <c:forEach var="pi" items="${message['lstPartItem']}">
              <option value="${pi.piName}">${pi.piId}--${pi.piName}</option>
            </c:forEach>
          </select>
        </div>
      </div>
    ...
    <div class="layui-form-item">
      ...
      <div class="layui-input-inline" style="width: 515px">
        <input type="text" class="layui-input" id="dnDate" name="dnDate" lay-verify="required" placeholder="yyyy-MM-dd" autocomplete="off">
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="dnName" name="dnName" lay-verify="required" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">计量单位</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="dnUnit" name="dnUnit" lay-verify="required" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="dnDoDesign" name="dnDoDesign" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">已完变更数量</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="dnDoChange" name="dnDoChange" lay-verify="number" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="dnUpDesign" name="dnUpDesign" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">对上计价变更数量</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="dnUpChange" name="dnUpChange" lay-verify="number" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="dnDownDesign" name="dnDownDesign" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">对下计价变更数量</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="dnDownChange" name="dnDownChange" lay-verify="number" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item layui-form-text">
      ...
      <div class="layui-input-inline" style="width: 515px">
        <textarea class="layui-textarea" id="dnInfo" name="dnInfo" placeholder="请输入内容"></textarea>
      </div>
    </div>
  </form>
</div>
...
<script>
    layui.use(['layer', 'laydate', 'form', 'table', 'jquery'], function(){
        ...
        table.render({
            ...
            ,where: { field: "dn_id", value: ""} //请求后端接口的条件，该处就是条件错误点，按照官方给出的代码示例，原先写成了 where: { key : { type: "all" } }，结果并不是我想的那样，如此写，key 将是后端的一个类作为参数，里面有 type 属性，如果误以为 key 是 Layui 提供的格式，那就大错特错了
            ,totalRow: true //开启合计行
            ...
            ,url: '${pageContext.request.contextPath}/demolition/searchDemolitionBody' // 使用url加载（可与data加载同时存在）
            ,cellMinWidth: 100
            ,event: true
            ,cols: [[{type:'numbers', title:'序号', fixed: true},{checkbox: true, fixed: true}
                ,{field:'dnId', title:'数量ID', width:100, sort: true, fixed: true}
                ,{field:'dnCode', title:'数量编码', Width:120, sort: true, fixed: true}
                ...
                ,{field:'dnDate', title:'施工日期', Width:100, sort: true, fixed: true}
                ,{field:'dnName', title:'数量名称', Width:120, sort: true, fixed: true}
                ,{field:'dnUnit', title:'计量单位'}
                ,{field:'dnDoDesign', title:'已完设计数量'}
                ,{field:'dnDoChange', title:'已完变更数量'}
                ,{field:'dnUpDesign', title:'对上计价设计数量'}
                ,{field:'dnUpChange', title:'对上计价变更数量'}
                ,{field:'dnDownDesign', title:'对下计价设计数量'}
                ,{field:'dnDownChange', title:'对下计价变更数量'}
                ,{field:'dnInfo', title:'数量备注'}
            ...
            switch(obj.event){
                ...
                case 'layTable_Add':
                    layer.open({
                        ...
                        ,yes: function(index){
                            $.getJSON('${pageContext.request.contextPath}/demolition/insertDemolitionBody',{
                                dnId: $('#dnId').val()
                                ,dnCode: $('#dnCode').val()
                                ,piId: $('#piId').val()
                                ,piName: $('#piName').val()
                                ...
                                ,dnName: $('#dnName').val()
                                ,dnUnit: $('#dnUnit').val()
                                ,dnDoDesign: $('#dnDoDesign').val()
                                ,dnDoChange: $('#dnDoChange').val()
                                ,dnUpDesign: $('#dnUpDesign').val()
                                ,dnUpChange: $('#dnUpChange').val()
                                ,dnDownDesign: $('#dnDownDesign').val()
                                ,dnDownChange: $('#dnDownChange').val()
                                ,dnInfo: $('#dnInfo').val()
                            },function(data){
                                //根据后台返回的参数，来进行判断
                                if(data.code === 1){
                                    ...
                                    table.reload('tableReload',{//重载表格
                                        method: 'post'
                                        ,url: '${pageContext.request.contextPath}/demolition/searchDemolitionBody'
                                        ,where: {
                                            field: 'dn_name'
                                            ,value: $('#dnName').val()
                                        }
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'detail'){
                ...
                layer.confirm('真的删除行么', function(index){
                    ...
                    layer.close(index);//关闭弹出层
                    //向服务端发送删除指令
                    $.getJSON('${pageContext.request.contextPath}/demolition/deleteDemolitionBody',{
                        id: data.dnId
                    },function(data){
                        //根据后台返回的参数，来进行判断
                        if(data.code === 1){
                ...
                layer.open({
                    ...
                    ,success: function () {
                        ...
                        $('#dnId').val(data.dnId);
                        $('#dnCode').val(data.dnCode);
                        $('#piId').val(data.piId);
                        $('#piName').val(data.piName);
                        $('#clId').val(data.clId);
                        $('#clCode').val(data.clCode);
                        $('#dnDate').val(data.dnDate);
                        $('#dnName').val(data.dnName);
                        $('#dnUnit').val(data.dnUnit);
                        $('#dnDoDesign').val(data.dnDoDesign);
                        $('#dnDoChange').val(data.dnDoChange);
                        $('#dnUpDesign').val(data.dnUpDesign);
                        $('#dnUpChange').val(data.dnUpChange);
                        $('#dnDownDesign').val(data.dnDownDesign);
                        $('#dnDownChange').val(data.dnDownChange);
                        $('#dnInfo').val(data.dnInfo);
                    }
                    ,yes: function(index){
                        $.getJSON('${pageContext.request.contextPath}/demolition/modifyDemolitionBody',{
                            dnId: $('#dnId').val()
                            ,dnCode: $('#dnCode').val()
                            ,piId: $('#piId').val()
                            ,piName: $('#piName').val()
