...
<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
  ...
  <table class="layui-hide" id="demolitionTable" lay-filter="test"></table>
...
<script>
      layui.config({
      ...
      }).use(['layer', 'dtable', 'laypage', 'table', 'jquery'], function(){
        ...
        // 渲染表格
        var table_demolition = dtable.render({
            elem: '#demolitionTable'
            ,method: 'post'
            ,title: '用户数据表'
            ,tree: {
                iconIndex: 2
            ...
            ,url: '${pageContext.request.contextPath}/demolition/toDemolitionBody'
            ,toolbar: '#toolbar'
            ,defaultToolbar: ['filter', 'print', 'exports']
            ,event: true
            ,cols: [[{type: 'numbers'},{type: 'checkbox'}
                ,{field: 'dnName', title: '菜单名称', minWidth: 400}
                ...
                ,{field:'dnCode', title:'数量编码', Width:100, sort: true, fixed: true}
        ...
        var table_costLayer = table.render({
            ...
            <%--,url: '${pageContext.request.contextPath}/demolition/restoreDemolitionBody'--%>
            ,toolbar: '#toolbar'
            ,defaultToolbar: ['filter', 'print', 'exports']
            ,event: true
        ...
        // 工具列点击事件
        dtable.on('tool(demolitionTable)', function (obj) {
            var event = obj.event;
            if (event === 'del') {
        ...
        // 头部工具栏点击事件
        dtable.on('toolbar(demolitionTable)', function (obj) {
            switch (obj.event) {
                case 'expandAll': // 全部展开
                    table_demolition.expandAll();
                    break;
                case 'foldAll': // 全部折叠
                    table_demolition.foldAll();
                    break;
                case 'expandData': // 展开指定
                    table_demolition.expand(2);
                    break;
                case 'foldData': // 展开指定
                    table_demolition.fold(2);
                    break;
                case 'setChecked': // 设置选中
                    table_demolition.expand(4);
                    table_demolition.setChecked([4]);
                    break;
                case 'getChecked': // 获取选中
                    layer.alert('<pre>' + JSON.stringify(table_demolition.checkStatus().map(function (d) {
                        return {
                            ctName: d.piName,
                            ctId: d.piId,
                ...
                case 'dataFilter': // 搜索
                    ...
                    if (keywords) {
                        table_demolition.filterData(keywords);
                    } else {
                        table_demolition.clearFilter();
                    }
                    break;
                case 'clearFilter': // 清楚搜索
                    ...
                    break;
                case 'tableReload': // 重载
                    table_demolition.reload();
                    break;
                case 'tableRefresh': // 刷新
                    table_demolition.refresh();
                    break;
            }
        });
