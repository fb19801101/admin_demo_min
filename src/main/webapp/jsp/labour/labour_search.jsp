  ...
  <div class="layui-inline">
    ...
    <div class="layui-input-inline">
      <select name="conditionField" id="demoCondition" lay-verify="required" lay-search="">
        ...
        <option selected="true" value="lb_id">数量ID</option>
        <option value="lb_code">数量编码</option>
        ...
        <option value="lb_date">施工日期</option>
        <option value="lb_name">数量名称</option>
      </select>
    </div>
  </div>
  ...
  <form class="layui-form" id="formLayer" onsubmit="return false">
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="lbId" name="lbId" lay-verify="number" placeholder="可以为空，系统默认自增！" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">数量编码</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="lbCode" name="lbCode" lay-verify="required" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      ...
      <div class="layui-input-inline" style="width: 515px">
        <input type="text" class="layui-input" id="lbDate" name="lbDate" lay-verify="required" placeholder="yyyy-MM-dd" autocomplete="off">
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="lbName" name="lbName" lay-verify="required" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">计量单位</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="lbUnit" name="lbUnit" lay-verify="required" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="lbDoDesign" name="lbDoDesign" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">已完变更数量</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="lbDoChange" name="lbDoChange" lay-verify="number" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="lbUpDesign" name="lbUpDesign" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">对上计价变更数量</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="lbUpChange" name="lbUpChange" lay-verify="number" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="lbDownDesign" name="lbDownDesign" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">对下计价变更数量</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="lbDownChange" name="lbDownChange" lay-verify="number" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item layui-form-text">
      ...
      <div class="layui-input-inline" style="width: 515px">
        <textarea class="layui-textarea" id="lbInfo" name="lbInfo" placeholder="请输入内容"></textarea>
      </div>
    </div>
  </form>
</div>
...
<script>
    layui.use(['layer', 'laydate', 'form', 'table', 'jquery'], function(){
        ...
        table.render({
            ...
            ,where: { field: "lb_id", value: ""} //请求后端接口的条件，该处就是条件错误点，按照官方给出的代码示例，原先写成了 where: { key : { type: "all" } }，结果并不是我想的那样，如此写，key 将是后端的一个类作为参数，里面有 type 属性，如果误以为 key 是 Layui 提供的格式，那就大错特错了
            ,totalRow: true //开启合计行
            ...
            ,url: '${pageContext.request.contextPath}/labour/searchLabourBody' // 使用url加载（可与data加载同时存在）
            ,cellMinWidth: 100
            ,event: true
            ,cols: [[{type:'numbers', title:'序号', fixed: true},{checkbox: true, fixed: true}
                ,{field:'lbId', title:'数量ID', width:100, sort: true, fixed: true}
                ,{field:'lbCode', title:'数量编码', Width:120, sort: true, fixed: true}
                ...
                ,{field:'lbName', title:'数量名称', Width:120, sort: true, fixed: true}
            ...
            switch(obj.event){
                ...
                case 'layTable_Add':
                    layer.open({
                        ...
                        ,yes: function(index){
                            $.getJSON('${pageContext.request.contextPath}/labour/insertLabourBody',{
                                lbId: $('#lbId').val()
                                ,lbCode: $('#lbCode').val()
                                ,piId: $('#piId').val()
                                ,piName: $('#piName').val()
                                ...
                                ,lbName: $('#lbName').val()
                                ,lbUnit: $('#lbUnit').val()
                                ,lbDoDesign: $('#lbDoDesign').val()
                                ,lbDoChange: $('#lbDoChange').val()
                                ,lbUpDesign: $('#lbUpDesign').val()
                                ,lbUpChange: $('#lbUpChange').val()
                                ,lbDownDesign: $('#lbDownDesign').val()
                                ,lbDownChange: $('#lbDownChange').val()
                                ,lbInfo: $('#lbInfo').val()
                            },function(data){
                                //根据后台返回的参数，来进行判断
                                if(data.code === 1){
                                    ...
                                    table.reload('tableReload',{//重载表格
                                        method: 'post'
                                        ,url: '${pageContext.request.contextPath}/labour/searchLabourBody'
                                        ,where: {
                                            field: 'lb_name'
                                            ,value: $('#lbName').val()
                                        }
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'detail'){
                ...
                layer.confirm('真的删除行么', function(index){
                    ...
                    layer.close(index);//关闭弹出层
                    //向服务端发送删除指令
                    $.getJSON('${pageContext.request.contextPath}/labour/deleteLabourBody',{
                        id: data.lbId
                    },function(data){
                        //根据后台返回的参数，来进行判断
                        if(data.code === 1){
                ...
                layer.open({
                    ...
                    ,success: function () {
                        ...
                        $('#lbId').val(data.lbId);
                        $('#lbCode').val(data.lbCode);
                        $('#piId').val(data.piId);
                        $('#piName').val(data.piName);
                        $('#clId').val(data.clId);
                        $('#clCode').val(data.clCode);
                        $('#lbDate').val(data.lbDate);
                        $('#lbName').val(data.lbName);
                        $('#lbUnit').val(data.lbUnit);
                        $('#lbDoDesign').val(data.lbDoDesign);
                        $('#lbDoChange').val(data.lbDoChange);
                        $('#lbUpDesign').val(data.lbUpDesign);
                        $('#lbUpChange').val(data.lbUpChange);
                        $('#lbDownDesign').val(data.lbDownDesign);
                        $('#lbDownChange').val(data.lbDownChange);
                        $('#lbInfo').val(data.lbInfo);
                    }
                    ,yes: function(index){
                        $.getJSON('${pageContext.request.contextPath}/labour/modifyLabourBody',{
                            lbId: $('#lbId').val()
                            ,lbCode: $('#lbCode').val()
                            ,piId: $('#piId').val()
                            ,piName: $('#piName').val()
