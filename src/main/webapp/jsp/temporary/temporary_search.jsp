  ...
  <div class="layui-inline">
    ...
    <div class="layui-input-inline">
      <select name="conditionField" id="demoCondition" lay-verify="required" lay-search="">
        ...
        <option selected="true" value="ty_id">数量ID</option>
        <option value="ty_code">数量编码</option>
        ...
        <option value="ty_date">施工日期</option>
        <option value="ty_name">数量名称</option>
      </select>
    </div>
  </div>
  ...
  <form class="layui-form" id="formLayer" onsubmit="return false">
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="tyId" name="tyId" lay-verify="number" placeholder="可以为空，系统默认自增！" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">数量编码</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="tyCode" name="tyCode" lay-verify="required" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline" style="width: 515px">
          <input type="text" class="layui-input" id="tyDate" name="tyDate" lay-verify="required" placeholder="yyyy-MM-dd" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="tyName" name="tyName" lay-verify="required" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">计量单位</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="tyUnit" name="tyUnit" lay-verify="required" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="tyDoDesign" name="tyDoDesign" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">已完变更数量</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="tyDoChange" name="tyDoChange" lay-verify="number" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="tyUpDesign" name="tyUpDesign" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">对上计价变更数量</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="tyUpChange" name="tyUpChange" lay-verify="number" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="tyDownDesign" name="tyDownDesign" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">对下计价变更数量</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="tyDownChange" name="tyDownChange" lay-verify="number" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item layui-form-text">
      ...
      <div class="layui-input-inline" style="width: 515px">
        <textarea class="layui-textarea" id="tyInfo" name="tyInfo" placeholder="请输入内容"></textarea>
      </div>
    </div>
  </form>
</div>
...
<script>
    layui.use(['layer', 'laydate', 'form', 'table', 'jquery'], function(){
        ...
        table.render({
            ...
            ,where: { field: "ty_id", value: ""} //请求后端接口的条件，该处就是条件错误点，按照官方给出的代码示例，原先写成了 where: { key : { type: "all" } }，结果并不是我想的那样，如此写，key 将是后端的一个类作为参数，里面有 type 属性，如果误以为 key 是 Layui 提供的格式，那就大错特错了
            ,totalRow: true //开启合计行
            ...
            ,url: '${pageContext.request.contextPath}/temporary/searchTemporaryBody' // 使用url加载（可与data加载同时存在）
            ,cellMinWidth: 100
            ,event: true
            ,cols: [[{type:'numbers', title:'序号', fixed: true},{checkbox: true, fixed: true}
                ,{field:'tyId', title:'数量ID', width:100, sort: true, fixed: true}
                ,{field:'tyCode', title:'数量编码', Width:120, sort: true, fixed: true}
                ...
                ,{field:'tyDate', title:'施工日期', Width:100, sort: true, fixed: true}
                ,{field:'tyName', title:'数量名称', Width:120, sort: true, fixed: true}
                ,{field:'tyUnit', title:'计量单位'}
                ,{field:'tyDoDesign', title:'已完设计数量'}
                ,{field:'tyDoChange', title:'已完变更数量'}
                ,{field:'tyUpDesign', title:'对上计价设计数量'}
                ,{field:'tyUpChange', title:'对上计价变更数量'}
                ,{field:'tyDownDesign', title:'对下计价设计数量'}
                ,{field:'tyDownChange', title:'对下计价变更数量'}
                ,{field:'tyInfo', title:'数量备注'}
            ...
            switch(obj.event){
                ...
                case 'layTable_Add':
                    layer.open({
                        ...
                        ,yes: function(index){
                            $.getJSON('${pageContext.request.contextPath}/temporary/insertTemporaryBody',{
                                tyId: $('#tyId').val()
                                ,tyCode: $('#tyCode').val()
                                ,piId: $('#piId').val()
                                ,piName: $('#piName').val()
                                ...
                                ,tyName: $('#tyName').val()
                                ,tyUnit: $('#tyUnit').val()
                                ,tyDoDesign: $('#tyDoDesign').val()
                                ,tyDoChange: $('#tyDoChange').val()
                                ,tyUpDesign: $('#tyUpDesign').val()
                                ,tyUpChange: $('#tyUpChange').val()
                                ,tyDownDesign: $('#tyDownDesign').val()
                                ,tyDownChange: $('#tyDownChange').val()
                                ,tyInfo: $('#tyInfo').val()
                            },function(data){
                                //根据后台返回的参数，来进行判断
                                if(data.code === 1){
                                    ...
                                    table.reload('tableReload',{//重载表格
                                        method: 'post'
                                        ,url: '${pageContext.request.contextPath}/temporary/searchTemporaryBody'
                                        ,where: {
                                            field: 'ty_name'
                                            ,value: $('#tyName').val()
                                        }
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'detail'){
                ...
                layer.confirm('真的删除行么', function(index){
                    ...
                    layer.close(index);//关闭弹出层
                    //向服务端发送删除指令
                    $.getJSON('${pageContext.request.contextPath}/temporary/deleteTemporaryBody',{
                        id: data.tyId
                    },function(data){
                        //根据后台返回的参数，来进行判断
                        if(data.code === 1){
                ...
                layer.open({
                    ...
                    ,success: function () {
                        ...
                        $('#tyId').val(data.tyId);
                        $('#tyCode').val(data.tyCode);
                        $('#piId').val(data.piId);
                        $('#piName').val(data.piName);
                        $('#clId').val(data.clId);
                        $('#clCode').val(data.clCode);
                        $('#tyDate').val(data.tyDate);
                        $('#tyName').val(data.tyName);
                        $('#tyUnit').val(data.tyUnit);
                        $('#tyDoDesign').val(data.tyDoDesign);
                        $('#tyDoChange').val(data.tyDoChange);
                        $('#tyUpDesign').val(data.tyUpDesign);
                        $('#tyUpChange').val(data.tyUpChange);
                        $('#tyDownDesign').val(data.tyDownDesign);
                        $('#tyDownChange').val(data.tyDownChange);
                        $('#tyInfo').val(data.tyInfo);
                    }
                    ,yes: function(index){
                        $.getJSON('${pageContext.request.contextPath}/temporary/modifyTemporaryBody',{
                            tyId: $('#tyId').val()
                            ,tyCode: $('#tyCode').val()
                            ,piId: $('#piId').val()
                            ,piName: $('#piName').val()
