...
<html>
<head>
  <title>分部分项信息列表</title>
  <link rel="stylesheet" href="${pageContext.request.contextPath}/res/js/layui-2.5.6/css/layui.css">
...
<script language="javascript" Charset="UTF-8">
    layui.config({
        ...
        <%--<c:forEach items="${message['list']}" var="pi">--%>
        <%--data.push({id:"${pi.piId}",--%>
        <%--    parentId:"${pi.piPid}",--%>
        <%--    title:"${pi.piName}",--%>
        <%--    level:"${pi.piLevel}",--%>
        <%--    spread:"${pi.piFold}" ? true:false,--%>
        <%--    checkArr:"${pi.piCheck}"});--%>
        <%--</c:forEach>--%>
        ...
        dtree.on("node(demoTree)", function(obj){
            ...
            layer.msg('id: '+json.nodeId+', node: '+json.context);
            ...
            parent.panelRight.location.href='${pageContext.request.contextPath}'+actionName+'?field=pi_id&id='+encodeURI(json.nodeId)+'&node='+encodeURI(json.context);
        });
