...
<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
  ...
  <table class="layui-hide" id="materialTable" lay-filter="test"></table>
...
<script>
      layui.config({
        ...
        // 渲染表格
        var table_material = dtable.render({
            elem: '#materialTable'
            ,method: 'post'
            ,title: '用户数据表'
            ,tree: {
            ...
            ,url: '${pageContext.request.contextPath}/material/toMaterialBody'
            ,toolbar: '#toolbar'
            ,defaultToolbar: ['filter', 'print', 'exports']
            ,event: true
            ,cols: [[{type: 'numbers'},{type: 'checkbox'}
                ,{field: 'mlName', title: '菜单名称', minWidth: 400}
                ...
                ,{field:'mlCode', title:'数量编码', Width:100, sort: true, fixed: true}
        ...
        var table_costLayer = table.render({
            ...
            ,totalRow: true //开启合计行
            ,cellMinWidth: 100
            ,where: { id: null, type: "物资层"}
            <%--,url: '${pageContext.request.contextPath}/material/restoreMaterialBody'--%>
            ,toolbar: '#toolbar'
            ,defaultToolbar: ['filter', 'print', 'exports']
            ,event: true
        ...
        // 工具列点击事件
        dtable.on('tool(materialTable)', function (obj) {
            var event = obj.event;
            if (event === 'del') {
        ...
        // 头部工具栏点击事件
        dtable.on('toolbar(materialTable)', function (obj) {
            switch (obj.event) {
                case 'expandAll': // 全部展开
                    table_material.expandAll();
                    break;
                case 'foldAll': // 全部折叠
                    table_material.foldAll();
                    break;
                case 'expandData': // 展开指定
                    table_material.expand(2);
                    break;
                case 'foldData': // 展开指定
                    table_material.fold(2);
                    break;
                case 'setChecked': // 设置选中
                    table_material.expand(4);
                    table_material.setChecked([4]);
                    break;
                case 'getChecked': // 获取选中
                    layer.alert('<pre>' + JSON.stringify(table_material.checkStatus().map(function (d) {
                        return {
                            ctName: d.piName,
                            ctId: d.piId,
                ...
                case 'dataFilter': // 搜索
                    ...
                    if (keywords) {
                        table_material.filterData(keywords);
                    } else {
                        table_material.clearFilter();
                    }
                    break;
                case 'clearFilter': // 清楚搜索
                    ...
                    break;
                case 'tableReload': // 重载
                    table_material.reload();
                    break;
                case 'tableRefresh': // 刷新
                    table_material.refresh();
                    break;
            }
        });
