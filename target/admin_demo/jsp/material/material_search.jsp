  ...
  <div class="layui-inline">
    ...
    <div class="layui-input-inline">
      <select name="conditionField" id="demoCondition" lay-verify="required" lay-search="">
        ...
        <option selected="true" value="ml_id">数量ID</option>
        <option value="ml_code">数量编码</option>
        ...
        <option value="ml_date">施工日期</option>
        <option value="ml_name">数量名称</option>
      </select>
    </div>
  </div>
  ...
  <form class="layui-form" id="formLayer" onsubmit="return false">
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="mlId" name="mlId" lay-verify="number" placeholder="可以为空，系统默认自增！" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">数量编码</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="mlCode" name="mlCode" lay-verify="required" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      ...
      <div class="layui-input-inline" style="width: 515px">
        <input type="text" class="layui-input" id="mlDate" name="mlDate" lay-verify="required" placeholder="yyyy-MM-dd" autocomplete="off">
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="mlName" name="mlName" lay-verify="required" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">计量单位</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="mlUnit" name="mlUnit" lay-verify="required" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="mlDoDesign" name="mlDoDesign" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">已完变更数量</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="mlDoChange" name="mlDoChange" lay-verify="number" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="mlUpDesign" name="mlUpDesign" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">对上计价变更数量</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="mlUpChange" name="mlUpChange" lay-verify="number" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="mlDownDesign" name="mlDownDesign" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">对下计价变更数量</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="mlDownChange" name="mlDownChange" lay-verify="number" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item layui-form-text">
      ...
      <div class="layui-input-inline" style="width: 515px">
        <textarea class="layui-textarea" id="mlInfo" name="mlInfo" placeholder="请输入内容"></textarea>
      </div>
    </div>
  </form>
</div>
...
<script>
    layui.use(['layer', 'laydate', 'form', 'table', 'jquery'], function(){
        ...
        table.render({
            ...
            ,where: { field: "ml_id", value: ""} //请求后端接口的条件，该处就是条件错误点，按照官方给出的代码示例，原先写成了 where: { key : { type: "all" } }，结果并不是我想的那样，如此写，key 将是后端的一个类作为参数，里面有 type 属性，如果误以为 key 是 Layui 提供的格式，那就大错特错了
            ,totalRow: true //开启合计行
            ...
            ,url: '${pageContext.request.contextPath}/material/searchMaterialBody' // 使用url加载（可与data加载同时存在）
            ,cellMinWidth: 100
            ,event: true
            ,cols: [[{type:'numbers', title:'序号', fixed: true},{checkbox: true, fixed: true}
                ,{field:'mlId', title:'数量ID', width:100, sort: true, fixed: true}
                ,{field:'mlCode', title:'数量编码', Width:120, sort: true, fixed: true}
                ...
                ,{field:'mlDate', title:'施工日期', Width:100, sort: true, fixed: true}
                ,{field:'mlName', title:'数量名称', Width:120, sort: true, fixed: true}
                ,{field:'mlUnit', title:'计量单位'}
                ,{field:'mlDoDesign', title:'已完设计数量'}
                ,{field:'mlDoChange', title:'已完变更数量'}
                ,{field:'mlUpDesign', title:'对上计价设计数量'}
                ,{field:'mlUpChange', title:'对上计价变更数量'}
                ,{field:'mlDownDesign', title:'对下计价设计数量'}
                ,{field:'mlDownChange', title:'对下计价变更数量'}
                ,{field:'mlInfo', title:'数量备注'}
            ...
            switch(obj.event){
                ...
                case 'layTable_Add':
                    layer.open({
                        ...
                        ,yes: function(index){
                            $.getJSON('${pageContext.request.contextPath}/material/insertMaterialBody',{
                                mlId: $('#mlId').val()
                                ,mlCode: $('#mlCode').val()
                                ,piId: $('#piId').val()
                                ,piName: $('#piName').val()
                                ...
                                ,mlName: $('#mlName').val()
                                ,mlUnit: $('#mlUnit').val()
                                ,mlDoDesign: $('#mlDoDesign').val()
                                ,mlDoChange: $('#mlDoChange').val()
                                ,mlUpDesign: $('#mlUpDesign').val()
                                ,mlUpChange: $('#mlUpChange').val()
                                ,mlDownDesign: $('#mlDownDesign').val()
                                ,mlDownChange: $('#mlDownChange').val()
                                ,mlInfo: $('#mlInfo').val()
                            },function(data){
                                //根据后台返回的参数，来进行判断
                                if(data.code === 1){
                                    ...
                                    table.reload('tableReload',{//重载表格
                                        method: 'post'
                                        ,url: '${pageContext.request.contextPath}/material/searchMaterialBody'
                                        ,where: {
                                            field: 'ml_name'
                                            ,value: $('#mlName').val()
                                        }
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'detail'){
                ...
                layer.confirm('真的删除行么', function(index){
                    ...
                    layer.close(index);//关闭弹出层
                    //向服务端发送删除指令
                    $.getJSON('${pageContext.request.contextPath}/material/deleteMaterialBody',{
                        id: data.mlId
                    },function(data){
                        //根据后台返回的参数，来进行判断
                        if(data.code === 1){
                ...
                layer.open({
                    ...
                    ,success: function () {
                        ...
                        $('#mlId').val(data.mlId);
                        $('#mlCode').val(data.mlCode);
                        $('#piId').val(data.piId);
                        $('#piName').val(data.piName);
                        $('#clId').val(data.clId);
                        $('#clCode').val(data.clCode);
                        $('#mlDate').val(data.mlDate);
                        $('#mlName').val(data.mlName);
                        $('#mlUnit').val(data.mlUnit);
                        $('#mlDoDesign').val(data.mlDoDesign);
                        $('#mlDoChange').val(data.mlDoChange);
                        $('#mlUpDesign').val(data.mlUpDesign);
                        $('#mlUpChange').val(data.mlUpChange);
                        $('#mlDownDesign').val(data.mlDownDesign);
                        $('#mlDownChange').val(data.mlDownChange);
                        $('#mlInfo').val(data.mlInfo);
                    }
                    ,yes: function(index){
                        $.getJSON('${pageContext.request.contextPath}/material/modifyMaterialBody',{
                            mlId: $('#mlId').val()
                            ,mlCode: $('#mlCode').val()
                            ,piId: $('#piId').val()
                            ,piName: $('#piName').val()
