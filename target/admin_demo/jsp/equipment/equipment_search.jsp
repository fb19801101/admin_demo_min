  ...
  <div class="layui-inline">
    ...
    <div class="layui-input-inline">
      <select name="conditionField" id="demoCondition" lay-verify="required" lay-search="">
        ...
        <option selected="true" value="et_id">数量ID</option>
        <option value="et_code">数量编码</option>
        ...
        <option value="et_date">施工日期</option>
        <option value="et_name">数量名称</option>
      </select>
    </div>
  </div>
  ...
  <form class="layui-form" id="formLayer" onsubmit="return false">
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="etId" name="etId" lay-verify="number" placeholder="可以为空，系统默认自增！" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">数量编码</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id=“etCode” name="etCode" lay-verify="required" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      ...
      <div class="layui-input-inline" style="width: 515px">
        <input type="text" class="layui-input" id="etDate" name="etDate" lay-verify="required" placeholder="yyyy-MM-dd" autocomplete="off">
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="etName" name="etName" lay-verify="required" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">计量单位</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="etUnit" name="etUnit" lay-verify="required" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="etDoDesign" name="etDoDesign" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">已完变更数量</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="etDoChange" name="etDoChange" lay-verify="number" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="etUpDesign" name="etUpDesign" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">对上计价变更数量</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="etUpChange" name="etUpChange" lay-verify="number" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="etDownDesign" name="etDownDesign" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">对下计价变更数量</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="etDownChange" name="etDownChange" lay-verify="number" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item layui-form-text">
      ...
      <div class="layui-input-inline" style="width: 515px">
        <textarea class="layui-textarea" id="etInfo" name="etInfo" placeholder="请输入内容"></textarea>
      </div>
    </div>
  </form>
</div>
...
<script>
    layui.use(['layer', 'laydate', 'form', 'table', 'jquery'], function(){
        ...
        table.render({
            ...
            ,where: { field: "et_id", value: ""} //请求后端接口的条件，该处就是条件错误点，按照官方给出的代码示例，原先写成了 where: { key : { type: "all" } }，结果并不是我想的那样，如此写，key 将是后端的一个类作为参数，里面有 type 属性，如果误以为 key 是 Layui 提供的格式，那就大错特错了
            ,totalRow: true //开启合计行
            ...
            ,url: '${pageContext.request.contextPath}/equipment/searchEquipmentBody' // 使用url加载（可与data加载同时存在）
            ,cellMinWidth: 100
            ,event: true
            ,cols: [[{type:'numbers', title:'序号', fixed: true},{checkbox: true, fixed: true}
                ,{field:'etId', title:'数量ID', width:100, sort: true, fixed: true}
                ,{field:'etCode', title:'数量编码', Width:120, sort: true, fixed: true}
                ...
                ,{field:'etDate', title:'施工日期', Width:100, sort: true, fixed: true}
                ,{field:'etName', title:'数量名称', Width:120, sort: true, fixed: true}
                ,{field:'etUnit', title:'计量单位'}
                ,{field:'etDoDesign', title:'已完设计数量'}
                ,{field:'etDoChange', title:'已完变更数量'}
                ,{field:'etUpDesign', title:'对上计价设计数量'}
                ,{field:'etUpChange', title:'对上计价变更数量'}
                ,{field:'etDownDesign', title:'对下计价设计数量'}
                ,{field:'etDownChange', title:'对下计价变更数量'}
                ,{field:'etInfo', title:'数量备注'}
            ...
            switch(obj.event){
                ...
                case 'layTable_Add':
                    layer.open({
                        ...
                        ,yes: function(index){
                            $.getJSON('${pageContext.request.contextPath}/equipment/insertEquipmentBody',{
                                etId: $('#etId').val()
                                ,etCode: $('#etCode').val()
                                ,piId: $('#piId').val()
                                ,piName: $('#piName').val()
                                ...
                                ,etName: $('#etName').val()
                                ,etUnit: $('#etUnit').val()
                                ,etDoDesign: $('#etDoDesign').val()
                                ,etDoChange: $('#etDoChange').val()
                                ,etUpDesign: $('#etUpDesign').val()
                                ,etUpChange: $('#etUpChange').val()
                                ,etDownDesign: $('#etDownDesign').val()
                                ,etDownChange: $('#etDownChange').val()
                                ,etInfo: $('#etInfo').val()
                            },function(data){
                                //根据后台返回的参数，来进行判断
                                if(data.code === 1){
                                    ...
                                    table.reload('tableReload',{//重载表格
                                        method: 'post'
                                        ,url: '${pageContext.request.contextPath}/equipment/searchEquipmentBody'
                                        ,where: {
                                            field: 'et_name'
                                            ,value: $('#etName').val()
                                        }
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'detail'){
                ...
                layer.confirm('真的删除行么', function(index){
                    ...
                    layer.close(index);//关闭弹出层
                    //向服务端发送删除指令
                    $.getJSON('${pageContext.request.contextPath}/equipment/deleteEquipmentBody',{
                        id: data.etId
                    },function(data){
                        //根据后台返回的参数，来进行判断
                        if(data.code === 1){
                ...
                layer.open({
                    ...
                    ,success: function () {
                        ...
                        $('#etId').val(data.etId);
                        $('#etCode').val(data.etCode);
                        $('#piId').val(data.piId);
                        $('#piName').val(data.piName);
                        $('#clId').val(data.clId);
                        $('#clCode').val(data.clCode);
                        $('#etDate').val(data.etDate);
                        $('#etName').val(data.etName);
                        $('#etUnit').val(data.etUnit);
                        $('#etDoDesign').val(data.etDoDesign);
                        $('#etDoChange').val(data.etDoChange);
                        $('#etUpDesign').val(data.etUpDesign);
                        $('#etUpChange').val(data.etUpChange);
                        $('#etDownDesign').val(data.etDownDesign);
                        $('#etDownChange').val(data.etDownChange);
                        $('#etInfo').val(data.etInfo);
                    }
                    ,yes: function(index){
                        $.getJSON('${pageContext.request.contextPath}/equipment/modifyEquipmentBody',{
                            etId: $('#etId').val()
                            ,etCode: $('#etCode').val()
                            ,piId: $('#piId').val()
                            ,piName: $('#piName').val()
