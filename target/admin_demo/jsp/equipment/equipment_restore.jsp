...
<script>
      layui.config({
        ...
        // 渲染表格
        var table_equipment = dtable.render({
            elem: '#equipmentTable'
            ,method: 'post'
            ,title: '用户数据表'
            ,tree: {
            ...
            ,url: '${pageContext.request.contextPath}/equipment/toEquipmentBody'
            ,toolbar: '#toolbar'
            ,defaultToolbar: ['filter', 'print', 'exports']
            ,event: true
            ,cols: [[{type: 'numbers'},{type: 'checkbox'}
                ,{field: 'etName', title: '菜单名称', minWidth: 400}
                ...
                ,{field:'etCode', title:'数量编码', Width:100, sort: true, fixed: true}
        ...
        var table_costLayer = table.render({
            ...
            <%--,url: '${pageContext.request.contextPath}/equipment/restoreEquipmentBody'--%>
            ,toolbar: '#toolbar'
            ,defaultToolbar: ['filter', 'print', 'exports']
            ,event: true
        ...
        // 工具列点击事件
        dtable.on('tool(equipmentTable)', function (obj) {
            var event = obj.event;
            if (event === 'del') {
        ...
        // 头部工具栏点击事件
        dtable.on('toolbar(equipmentTable)', function (obj) {
            switch (obj.event) {
                case 'expandAll': // 全部展开
                    table_equipment.expandAll();
                    break;
                case 'foldAll': // 全部折叠
                    table_equipment.foldAll();
                    break;
                case 'expandData': // 展开指定
                    table_equipment.expand(2);
                    break;
                case 'foldData': // 展开指定
                    table_equipment.fold(2);
                    break;
                case 'setChecked': // 设置选中
                    table_equipment.expand(4);
                    table_equipment.setChecked([4]);
                    break;
                case 'getChecked': // 获取选中
                    layer.alert('<pre>' + JSON.stringify(table_equipment.checkStatus().map(function (d) {
                        return {
                            ctName: d.piName,
                            ctId: d.piId,
                ...
                case 'dataFilter': // 搜索
                    ...
                    if (keywords) {
                        table_equipment.filterData(keywords);
                    } else {
                        table_equipment.clearFilter();
                    }
                    break;
                case 'clearFilter': // 清楚搜索
                    ...
                    break;
                case 'tableReload': // 重载
                    table_equipment.reload();
                    break;
                case 'tableRefresh': // 刷新
                    table_equipment.refresh();
                    break;
            }
        });
