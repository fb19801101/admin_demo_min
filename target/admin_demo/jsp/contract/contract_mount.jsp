...
<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
  <legend>清单信息挂载</legend>
  <table class="layui-hide" id="demoContractTable"></table>
</fieldset>
...
<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
  ...
  <table class="layui-hide" id="demoGuidanceTable"></table>
</fieldset>
...
<script>
    ...
    layui.use(['layer', 'table', 'jquery'], function(){
        ...
        table.render({
            ...
            ,totalRow: true //开启合计行
            ,url: '${pageContext.request.contextPath}/contract/mountContractBody' // 使用url加载（可与data加载同时存在）
            ,cellMinWidth: 100
            ,event: true
        ...
        table.render({
            ...
            ,where: {ciId: urlId} //请求后端接口的条件，该处就是条件错误点，按照官方给出的代码示例，原先写成了 where: { key : { type: "all" } }，结果并不是我想的那样，如此写，key 将是后端的一个类作为参数，里面有 type 属性，如果误以为 key 是 Layui 提供的格式，那就大错特错了
            ,totalRow: true //开启合计行
            ,url: '${pageContext.request.contextPath}/contract/mountBudgetBody' // 使用url加载（可与data加载同时存在）
            ,cellMinWidth: 100
            ,event: true
            ,cols: [[{type:'numbers', title:'序号', fixed: true}
                ,{field:'bgId', title:'概算ID', width:100, sort: true, fixed: true, totalRowText: '金额合计'}
                ,{field:'bgCode', title:'定额编码', minWidth:120, sort: true}
        ...
        table.render({
            ...
            ,where: {ciId: urlId} //请求后端接口的条件，该处就是条件错误点，按照官方给出的代码示例，原先写成了 where: { key : { type: "all" } }，结果并不是我想的那样，如此写，key 将是后端的一个类作为参数，里面有 type 属性，如果误以为 key 是 Layui 提供的格式，那就大错特错了
            ,totalRow: true //开启合计行
            ,url: '${pageContext.request.contextPath}/contract/mountGuidanceBody' // 使用url加载（可与data加载同时存在）
            // ,data:data //数据接口
            ,cellMinWidth: 80
            ,event: true
            ,cols: [[{type:'numbers', title:'序号', fixed: true}
        ...
        table.render({
            ...
            ,where: {field: urlField, value: urlId} //请求后端接口的条件，该处就是条件错误点，按照官方给出的代码示例，原先写成了 where: { key : { type: "all" } }，结果并不是我想的那样，如此写，key 将是后端的一个类作为参数，里面有 type 属性，如果误以为 key 是 Layui 提供的格式，那就大错特错了
            ,totalRow: true //开启合计行
            ,url: '${pageContext.request.contextPath}/contract/mountCostLayerBody' // 使用url加载（可与data加载同时存在）
            ,cellMinWidth: 100
            ,event: true
            ,cols: [[{type:'numbers', title:'序号', fixed: true}
